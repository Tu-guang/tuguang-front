          ...
          <a-row :gutter="48">
            <a-col :md="4" :sm="24">
              <a-form-item label="id">
                <a-input v-model="queryParam.id" placeholder="接口id" />
              </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
            ...
            <template v-if="advanced">
              <a-col :md="8" :sm="24">
                <a-form-item label="发布者">
                  <a-input-number v-model="queryParam.userId" style="width: 100%" placeholder="请输入发布者" />
                </a-form-item>
              </a-col>
              <a-col :md="8" :sm="24">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return interfacelist(requestParameters)
          .then(res => {
            return res.data
          })
      },
  ...
  methods: {
        ...
        if (!errors) {
          ...
          if (values.id > 0) {
            ...
            new Promise((resolve, reject) => {
              ...
              // 重置表单数据
              form.resetFields()
              updateInterfaceInfo(values).then((res) => {
                if (res.code === 0) {
                  // 刷新表格
            ...
            new Promise((resolve, reject) => {
              ...
              // 重置表单数据
              form.resetFields()
              addlnterfacelnfo(values).then((res) => {
                if (res.code === 0) {
                  // 刷新表格
                  this.$refs.table.refresh()
    ...
    handleDel (record) {
      deleteInterfaceInfo({ id: record.id }).then((res) => {
        if (res.code === 0) {
